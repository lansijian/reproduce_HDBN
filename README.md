...
首先对于原来的训练集和测试集进行转化，使用比赛方提供的训练集，然后使用data中的transform_2d.py和transform.py将原来的train、vel与test的npy文件和对应的label文件（test的label是自己创建的全部是0的文件）转化为适合训练集的npz文件，然后再于Mix_GCN中修改config文件夹中对应的训练和测试的文件路径，然后就进行训练，得到每一个训练的pkl文件后，用主文件夹下中的pkl_npy.py将每一个对应的模型转化为对应的置信度文件npy，之后利用主文件夹中的ensemble_npy_eval_A_2.py对置信度文件进行融合的权重分配（PS：我们使用了两个仓库中的置信度文件进行融合），之后找到最佳的权重分配，用训练得到的最优的模型pt文件测试生成B测试集对应的pkl文件，再同理转换为npy文件，在ensemble_npy_eval_B.py中利用最佳的权重分配对B测试集的置信度文件进行融合得到最终的置信度文件combined.npy，最后修改置信度名字为pred.npy，至此结束
...
最终版本采用了单目标优化模型来搜寻最佳的模型融合权重，我们以准确度最大为优化目标，采用powell方法对问题求解，使用的文件和参数如下
```
files = [
